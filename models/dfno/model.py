...
class AxisSpectralConv(nn.Module):
    ...
    def __init__(self, width: int, modes: int):
        ...
        self.weights_low = nn.Parameter(scale * torch.rand(self.width, self.width, self.modes, 1, dtype=torch.cfloat))
        self.weights_high = nn.Parameter(scale * torch.rand(self.width, self.width, self.modes, 1, dtype=torch.cfloat))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out_ft = torch.zeros(x.shape[0], self.width, x.shape[2], x_ft.shape[-1], dtype=torch.cfloat, device=x.device)
        out_ft[..., :n_modes] = self.compl_mul1d(x_ft[..., :n_modes], self.weights_low[..., :n_modes, :])
        out_ft[..., -n_modes:] = self.compl_mul1d(x_ft[..., -n_modes:], self.weights_high[..., :n_modes, :])
...
class AxisBlock(nn.Module):
    ...
    def apply_local(layer: nn.Conv1d, x: torch.Tensor) -> torch.Tensor:
        b, c, p, d = x.shape
        return layer(x.permute(0, 2, 1, 3).reshape(b * p, c, d)).reshape(b, p, c, d).permute(0, 2, 1, 3)
...
class AxisDecomp(nn.Module):
    ...
    def forward(self, h: torch.Tensor):
        if tuple(h.shape[2:]) != self.grid:
            raise ValueError(f'AxisDecomp built for grid {self.grid}, got {tuple(h.shape[2:])}')
        b, c = h.shape[:2]
        factors = []
...
class DecomposedFNOCore(nn.Module):
    ...
    def __init__(self, n_modes, hidden_channels: int, in_channels: int, out_channels: int, n_layers: int=4, decomposition_terms: int=20, grid_size=None, decomp_grid=None, spatial_dims=None, **kwargs):
        super().__init__()
        ...
        modes = normalize_modes(modes, self.spatial_dims)
        self.lift = nn.Linear(self.in_channels + self.spatial_dims, self.width)
        self.axis_decomp = AxisDecomp(self.terms, self.grid)
        self.blocks = nn.ModuleList([AxisBlock(self.width, modes) for _ in range(int(n_layers))])
        self.project = nn.Sequential(nn.Linear(self.width, 128), nn.GELU(), nn.Linear(128, self.out_channels))
    ...
    def reconstruct(self, factors):
        ...
        if self.spatial_dims == 3:
            return torch.einsum('bcpt,bcpj,bcpk->bctjk', factors[0], factors[1], factors[2])
        if self.spatial_dims == 4:
            return torch.einsum('bcpi,bcpj,bcpk,bcpl->bcijkl', factors[0], factors[1], factors[2], factors[3])
        raise ValueError(f'Unsupported spatial_dims={self.spatial_dims}')

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != self.spatial_dims + 2:
            raise ValueError(f'Expected [B,C,*spatial] with {self.spatial_dims} dims, got {tuple(x.shape)}')
        if tuple(x.shape[2:]) != self.grid:
            raise ValueError(f'Expected spatial {self.grid}, got {tuple(x.shape[2:])}')
        b = x.shape[0]
        x_cl = x.movedim(1, -1)
        h = self.lift(torch.cat((x_cl, self.coordinate_grid(b, x.device, x.dtype)), dim=-1))
        h = h.movedim(-1, 1)
        factors = self.axis_decomp(h)
        for block in self.blocks:
            factors = block(factors)
        field = self.reconstruct(factors)
