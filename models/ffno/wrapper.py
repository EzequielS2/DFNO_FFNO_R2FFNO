...
from .mesh import FNOFactorizedMesh

class FFNO(nn.Module):

    def __init__(self, n_modes, hidden_channels, in_channels, out_channels, n_layers=4, share_weight=True, factor=4, ff_weight_norm=True, n_ff_layers=2, layer_norm=False, padding=8, spatial_dims=None, **kwargs):
        super().__init__()
        modes, spatial_dims = resolve_modes(n_modes, spatial_dims)
        self.spatial_dims = int(spatial_dims)
        self._init_kwargs = dict(n_modes=list(n_modes) if not isinstance(n_modes, int) else n_modes, hidden_channels=int(hidden_channels), in_channels=int(in_channels), out_channels=int(out_channels), n_layers=int(n_layers), share_weight=bool(share_weight), factor=int(factor), ff_weight_norm=bool(ff_weight_norm), n_ff_layers=int(n_ff_layers), layer_norm=bool(layer_norm), padding=int(padding), spatial_dims=self.spatial_dims)
        self.core = FNOFactorizedMesh(n_modes=modes, width=int(hidden_channels), input_dim=int(in_channels) + self.spatial_dims, output_dim=int(out_channels), n_layers=int(n_layers), share_weight=bool(share_weight), factor=int(factor), ff_weight_norm=bool(ff_weight_norm), n_ff_layers=int(n_ff_layers), layer_norm=bool(layer_norm), padding=int(padding))
